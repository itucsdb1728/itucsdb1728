...
class SchoolClass:
    ...
    def get_class_school(self, class_id):
        
        with dbapi2.connect(self.dsn) as connection:
            cursor = connection.cursor()
            query = """SELECT school_id FROM school_class_table WHERE
                        (class_id = (%s)) """
            param = (class_id,)
    ...
    def get_schools_all_classes(self, school_id):
        
        with dbapi2.connect(self.dsn) as connection:
            cursor = connection.cursor()
            query = """SELECT class_id FROM school_class_table WHERE
                        (school_id = (%s)) """
            param = (school_id,)
